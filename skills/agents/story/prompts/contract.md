...
- Important-character dialogue from side threads must remain source-backed by validated side `actor.outputs.json`/trace provenance in `story.input.json`, except for explicitly marked special-scene temporary subGM portrayal inside that side-thread memory/dream/symbolic scene. Do not turn ordinary subGM notes into independent dialogue boxes.
