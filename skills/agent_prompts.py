...
def _input_analyst_prompt(context: Dict[str, Any]) -> str:
    ...
    return _base_prompt(
        ...
        "`replay.plan`, `replay.execute`, `card.patch_data`, and `character.rename`; unsupported capability names may "
        ...
        "`retcon.consult -> story`, `replay.plan -> replay`, `replay.execute -> replay`, "
        "`card.patch_data -> card-data`, `character.rename -> memory`. "
        "For `assets.generate_image`, payload may include `asset_requirement` "
        "with `scene_illustration_each_round: true`, `characters`, "
        "`character_appearances`, `reference_policy`, `reference_candidates`, `art_style`, and `planner_hints`. "
        "Use `character_appearances` when a character's current scene appearance differs "
        "from their default design, such as transformation, disguise, uniform, injury, "
        "or battle form; each item should include `name`, `appearance_state`, and a "
        "brief `description` so assets-ui can choose or create the matching character reference. "
        "Use `art_style` only when the user explicitly asks to set or adjust the illustration style; "
        "when no reference images exist, assets-ui will ask the image model to infer a suitable style from the story. "
        "Use these fields when the user explicitly asks for persistent "
        "illustrations or image consistency; do not infer illustration requests "
        "from ordinary role-play actions. "
        "Use `character.rename` when the current player placeholder name or an important character's "
        "name should change while preserving the same identity; payload uses `from_name`, `to_name`, "
        "and optional `actor_id`, and this is distinct from switching the protagonist mapping to "
        "another existing character. `source.change_request` must use "
        "`authorization_gate: \"allowSourceCodeSelfRepair\"`; `replay.plan` and "
        "`replay.execute` must use `authorization_gate: \"none\"`; `card.patch_data` "
        "must use `authorization_gate: \"manual_confirmation\"`. For a "
        "continuity retcon / rollback replay, you must not rely only on "
        "`narrative_directives.rewrite_previous_output`; that flag is only a "
        "story lightweight derived edit signal and must not trigger implicit "
        "rollback, and must emit `replay.plan` with payload fields including "
        "`backup_id`, `affected_inputs`, and `plan_id`; when the replay should "
        "start immediately, also emit a paired `replay.execute` request that "
        "points to the same `plan_id`. `replay.plan` materializes a `.replay` "
        "session and audit artifact; `replay.execute` executes or resumes that "
        "session. `retcon.consult` remains consultation-only and is not the "
        "same as executing replay. Keep legacy "
...
def _gm_prompt(context: Dict[str, Any]) -> str:
    ...
    return _base_prompt(
        ...
        "flashback/memory/dream/retcon recap scene.\n"
        "\nSpecial scene memory disclosure: unless the plot explicitly requires mind infiltration "
        "or direct mental perception, GM temporarily portrays every important character appearing "
        "in the special scene instead of calling their actor agents inside that recalled, dreamed, "
        "or symbolic scene. When the special scene ends, judge each appearing important character "
        "separately: whether the target character should know anything, whether that knowledge is "
        "clear memory, vague impression, or no disclosure, and which parts belong only in GM-only "
        "foreshadowing/objective memory. Use `actor_calls[].prompt` only for the selected "
        "second-person disclosure to that specific actor. A dream-forgotten scene may leave the "
        "dreamer only a vague impression while the dream content remains GM foreshadowing; an "
        "important character's behavior inside another person's dream should normally not be "
        "disclosed to that character.\n"
...
def _subgm_prompt(context: Dict[str, Any]) -> str:
    ...
    return _base_prompt(
        ...
        "of calling their actor agents inside that special scene. When the scene ends, report "
        "disclosure recommendations to GM for each appearing important character: clear memory, "
        "vague impression, no disclosure, and any GM-only foreshadowing/objective-memory residue. "
        "Only use `actor_calls[].prompt` for a target actor when the side-thread boundary says that "
        "actor should actually receive a second-person disclosure.\n"
    )
...
def _story_prompt(run_summary: Dict[str, Any]) -> str:
    ...
    return _base_prompt(
        ...
        contract_notes=(
            ...
            "rewritten as external visible pressure, not invented as that actor's choice. In an "
            "explicit flashback/memory/dream/symbolic/retcon recap, special-scene temporary GM/subGM "
            "portrayal is an allowed source for the special scene only; outside that special scene, "
            "current reality returns to the actor-source-backed rule. "
