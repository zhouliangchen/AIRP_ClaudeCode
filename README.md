...
│  ├─ agent_snapshots.py    # save-root backup/ 备份与回滚辅助
...
桌面端会显示正文、插图、行动选项、输入框和侧栏配置。移动端默认只保留基础背景、对话历史、插图、下一步行动选择题和自定义输入框；右上角设置按钮会展开覆盖式侧边栏，用于调整文风、NSFW、字数、自我修复模式、源码自修复开关、调试模式和当前目标等进阶选项。下一步行动选择题在桌面端和移动端都会显示在最后一次正文输出之后，随正文一起滚动；玩家选择行动或成功提交本轮输入后，当前轮行动建议会立即隐藏。
...
运行时 RP agent 默认优先使用 cc_switch Claude Code 本地反代；cc_switch 只在 AIRP 中配置 `enabled` 和 `service_url`，API key、模型名和上游 provider 映射由 CC Switch/Claude Code 本地配置维护。文本模型分为三档：默认核心模型 `core`，projection agent 使用审查模型 `review`，player/character agent 使用角色模型 `actor`。cc_switch 三档分别读取 Claude Code/CC Switch 的 Opus、Sonnet、Haiku 映射；若 cc_switch 不可用或关闭，可启用 OpenAI-compatible 文本 API 作为兜底，并分别配置 `core`、`review`、`actor` 三档的 `base_url`、`api_key` 和 `model`。

前端侧栏的“API 设置”会统一管理 cc_switch、OpenAI-compatible 文本 API 和图片生成 API，并写入 `skills/styles/llm_settings.frontend.json`。本地兜底配置统一写入 `skills/styles/llm_settings.local.json`。三类配置的字段优先级统一为：前端设置 > 环境变量 > 本地配置文件；代码不会注入 API 默认值，三层均未设置时，前端 API 设置页会显示配置错误。环境变量分组为：`AIRP_CC_SWITCH_ENABLED` / `AIRP_CC_SWITCH_SERVICE_URL`；`AIRP_OPENAI_COMPATIBLE_ENABLED`，以及 `AIRP_OPENAI_COMPATIBLE_CORE_BASE_URL` / `AIRP_OPENAI_COMPATIBLE_CORE_API_KEY` / `AIRP_OPENAI_COMPATIBLE_CORE_MODEL`、`AIRP_OPENAI_COMPATIBLE_REVIEW_BASE_URL` / `AIRP_OPENAI_COMPATIBLE_REVIEW_API_KEY` / `AIRP_OPENAI_COMPATIBLE_REVIEW_MODEL`、`AIRP_OPENAI_COMPATIBLE_ACTOR_BASE_URL` / `AIRP_OPENAI_COMPATIBLE_ACTOR_API_KEY` / `AIRP_OPENAI_COMPATIBLE_ACTOR_MODEL`；图片生成 API 继续使用 `AIRP_IMAGE_GENERATION_BASE_URL` / `AIRP_IMAGE_GENERATION_API_KEY` / `AIRP_IMAGE_GENERATION_MODEL`。
...
python skills/image_generate.py "<卡片文件夹>" --prompt "rainy seaside convenience store" --kind scene --target scene_illustration --async --job-id round-001 --reference ".\\characters\\初春\\初春.png" --output-path ".\\generated\\images\\scene_illustration.png"
```

前端配置文件为 `skills/styles/llm_settings.frontend.json`，本地兜底配置文件为 `skills/styles/llm_settings.local.json`，图片字段均位于 `image_generation`，支持 `api_key`、`base_url`、`model`。当前建议的初始值已写入本地配置文件；代码中不再硬编码这些值。旧的 `image_config.local.json`、`.image_api.json` 以及旧 `OPENAI_API_KEY` / `OPENAI_BASE_URL` / `IMAGE_MODEL` 不再作为图片生成配置读取。`image_generate.py` 也支持按需传入 `--job-id`、`--reference` 和 `--output-path`，分别用于补充任务追踪、显式参考图和输出落盘路径。生成图片会写入当前卡片文件夹的 `generated/images/` 和 `.card_assets.json`，脚本会重建 `content.js`，已打开的浏览器会自动看到新图片。
...
当用户在指令通道主动请求系统能力、UI/图片更新、剧情回滚/重演协商、存档角色数据调整或源码功能实现时，input analyst 可以在 `input_analysis.output.json.capability_requests[]` 中写入 agent 自主判断后的能力请求。GM、subGM、story、critic 的 `asset_requests[]` 会被运行时转换为同一类 `assets.generate_image` capability，其中 subGM 只允许新增，不允许修改或删除。Python 不再通过固定关键词或固定 request type 判断路由语义，只根据 capability registry 校验能力是否存在、授权门是否满足、目标 agent/executor 是否可用，以及 artifact 路径、backup、projection、delivery gate 等安全不变量。`assets.generate_image` 会被转为 `assets_task`，由 runtime pump 与 assets-ui runtime planner 同步 UI schema、postprocess contract、任务状态和持久化资产需求；相关状态会落到 `generated/jobs/`、`artifacts/assets_ui/` 和扩展后的 `.card_assets.json`。`replay.plan` 会物化 `.replay/sessions/<plan_id>` replay session 和审计 artifact，不直接回滚；`replay.execute` 已接入生产重演执行器，可执行或恢复 `.replay/` replay session；`.retcon_replay.json` 已废弃并保持 no-op，`rewrite_previous_output` 不再触发隐式回滚；源码能力 `source.change_request` 仍必须经过 `allowSourceCodeSelfRepair` 授权，未授权时 blocked 且不自动改源码。
...
`memory/` 是 GM 维护的客观记忆和可读上下文区，用于记录世界、角色客观设定、剧情事实、规则和其他非 actor 自我记忆材料，并随 `backup/` 回滚一起纳入恢复边界。player/character 的自我记忆权威不在 `memory/` 下，而在根目录 `characters/<角色名>/` 下的 `profile.md`、`long_term_memories.md`、`key_memories.json` 和 `short_term_memories.md`。
...
成功交付到前端后，系统会执行回合级 agent lifecycle cleanup：仍处于 `running`、`merging`、`needs_gm`、`blocked` 或 `max_steps` 的 subGM 支线会被暂停并释放角色占用；未完成的支线不会被标记为 completed，而是保留 `next_resume_point` 供之后由主 GM 恢复。默认 live path 不再给 subGM 支线设置 `max_steps` 截断，只有测试或显式调用传入该参数时才会产生 `max_steps` 状态。player/character actor 调度前会重新计算 `context_version`，因此人格、背景、记忆或目标文件被更新后，下一次调度会读取最新上下文；已经在运行的 actor 调用不会被强制中断。
