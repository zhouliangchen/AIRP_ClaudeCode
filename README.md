...
projection 采用两层边界：LLM projection agent 负责语义层的 actor-facing review 与局部改写，把 actor call 渲染成沉浸式上下文；Python 只保留最小确定性协议门，校验 actor id、source call id、ACL、artifact provenance、retry 和 rollback 不变量。player/character 收到的是 immersive context；隐藏事实、projection feedback 和 misconception 标签都不是 actor-facing 内容，错误认知应作为角色自己的主观记忆或信念保留。
