...
│  ├─ control_plane_smoke.py # 无 live model 的控制面冒烟测试
...
`agent_workflow.py` 可根据 `.agent_runs/<round>/manifest.json` 判断下一步应补齐 agent 产物、生成 `story.input.json`、分派 story/critic，还是运行交付门禁。`control_plane_smoke.py` 使用临时目录构造一轮多 agent 控制面流程，不调用 live model，用于快速验证 artifact、trace、memory delta/summary 和交付路径。
