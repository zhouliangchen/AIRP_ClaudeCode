...
- If the bootstrap result says `turn_generated`, `rp_generate_cli.py` has already run `round_prepare.py`, dispatched the input analyst, applied `input_analysis.output.json`, called the required runtime agents through the configured LLM provider, written `gm.output.json`, `actor.outputs.json`, `story.output.json`, and `critic.report.json`, and invoked `round_deliver.py`. Do not generate this turn again.
...
- Runtime agent dispatch uses local Python code and `llm_runner.run_llm_agent()`. Claude Code remains the entrypoint and maintainer, not the routine creative writer.
- `rp_generate_cli.py` is the deterministic wrapper that invokes the runtime agent path without relying on this top-level prompt to manually write artifacts.
...
- Then execute those local instructions directly: determine startup mode, run import/start-server scripts when needed, read `skills/styles/import_context.txt` or `round_context.txt`, and coordinate runtime artifacts through `.agent_runs/<round>/`.
- For a new imported card with a prefilled `skills/styles/response.txt` and empty `chat_log.json`, deliver the opening with `python "{ROOT}/skills/frontend/handler.py" "<card_folder>" --opening` before waiting for player input.
- For pending player input in the fallback path, run `python "{ROOT}/skills/round_prepare.py" "<card_folder>" "{ROOT}"`, then run `python "{ROOT}/skills/rp_generate_cli.py" "<card_folder>" "{ROOT}"`. The CLI dispatches the input analyst and applies `input_analysis.output.json` before the GM loop and actor outputs are generated.
- The main agent handles orchestration, script execution, runtime agent dispatch, system iteration, and final quality gates. Routine narration and roleplay must be delegated to runtime agents.
