...
Use state-machine progress updates for input analysis, GM loop, actor dispatch, subGM dispatch, Story, Critic, delivery, memory, and lifecycle cleanup. Progress/reply status remains outside postprocess; do not ask the postprocess agent to write `progress.json` or invent new progress state IDs without adding them to `skills/round_state.py`. After delivery, ensure lifecycle cleanup has recorded paused side threads and actor context warnings when applicable.
