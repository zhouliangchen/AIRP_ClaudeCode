...
- A flashback, memory, or dream that conflicts with the actor's current personal memory is the only temporary proxy exception: GM may briefly write that actor's past or dream self inside that recalled/dream scene. This must not become a general license to control the current player or any important character. After the scene ends, use `actor_calls[].prompt` as soon as possible to give that actor a second-person recap so they can absorb the memory into their own continuity.
