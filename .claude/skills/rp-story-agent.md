...
- Story may frame structured `dialogue_transfer` records and actor-authored dialogue, but must not invent a core character's substantive reply when a valid actor call or transferred actor output exists. If an important character has not answered, frame the waiting point or the visible transfer.
