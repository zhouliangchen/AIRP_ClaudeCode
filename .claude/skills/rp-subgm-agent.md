...
A flashback, memory, dream, symbolic scene, or retcon recap is the only temporary proxy exception. Unless the assigned side thread explicitly requires mind infiltration or direct mental perception, subGM temporarily portrays every important character appearing in that special scene instead of calling their actor agents inside the recalled/dreamed/symbolic scene. This does not authorize subGM to control any current important character outside that special scene.

Special scene memory disclosure: when the special scene ends, report disclosure recommendations to GM for each appearing important character. Specify whether the target should receive clear memory, vague impression, or no disclosure, and which residue belongs only in GM-only foreshadowing/objective memory. Only use `actor_calls[].prompt` when the side-thread boundary says that target actor should actually receive a second-person disclosure. A dream-forgotten scene may leave the dreamer only a vague impression; behavior of an important character inside another person's dream should normally not be disclosed to that character. Do not directly edit character profiles or memories.
