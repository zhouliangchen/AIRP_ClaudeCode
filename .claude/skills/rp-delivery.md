...
3. `round_deliver.py` performs mechanical artifact checks, rebuilds `story.input.json` if needed, validates `postprocess.output.json.core`, mirrors critic-approved `story.output.json.content` to `skills/styles/response.txt`, invokes `handler.py`, ingests subagent memory deltas, schedules `post_round_memory_jobs/*.job.json` for participating actors, ingests completed `post_round_memory_jobs/*.summary.json`, and marks the manifest `delivered`. It does not reject prose for length, perspective, style, NSFW, or other narrative quality concerns; those belong to the critic stage. Post-round memory failures are reported in `post_round_memory_jobs.status` without blocking an already approved text delivery.
