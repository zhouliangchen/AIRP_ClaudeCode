...
Task 7 已把 deterministic control-plane smoke 扩展到已实现的硬化行为：GM actor call 中来自 `user_instruction_channel` 的隐藏短语会在进入 actor/story-facing loop output 前清理；主 GM 可通过 `character_promotions` 在当前回合提升 `SuLi`；周期性 actor 记忆摘要会写入 `long_term.md`、`key_memories.md`、`short_term.md` 和 `goals.json` 结构化记忆桶；subGM side-thread runner 已支持主 GM 创建、运行、暂停、完成和读取支线消息，且禁止 subGM 直接 promotion、派生 subGM、包含玩家角色或直接改边界。当前 subGM 行为以 `docs/superpowers/specs/2026-06-19-subgm-side-thread-design.md` 为准。
...
- 本轮不重写前端交互布局。
- 本轮只记录交互式 GM 主回路的原始设计；已落地的 subGM side-thread runner 现状见 `2026-06-19-subgm-side-thread-design.md`。
...
## subGM side-thread runner

subGM side-thread runner 的当前实现已经从预留能力升级为独立设计说明。支线术语、权限、生命周期、消息模型、角色占用规则、artifact 布局、Story/Critic 处理和剩余限制见 `docs/superpowers/specs/2026-06-19-subgm-side-thread-design.md`。本文保留交互式 GM 主回路的原始设计语境，不在此重复维护 subGM 细节。
