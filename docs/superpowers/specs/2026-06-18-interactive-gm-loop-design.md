...
- 本轮不重写前端交互布局。
- 本轮不完整实现 subGM side-thread runner。
...
## subGM side-thread runner 预留

本轮不完整实现 subGM side-thread runner，但协议预留多实例能力：

- GM 可在未来创建多个 `subGM:<thread_id>`。
- 每个 subGM 负责一个并行支线，拥有自己的 `side_threads/<thread_id>/interaction.trace.json`。
