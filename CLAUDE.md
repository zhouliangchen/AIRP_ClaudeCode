...
**玩家输入权威原则**。玩家历次输入是最高优先级事实源，`.player_inputs.jsonl` 中的 `raw_text` / `display_text` 不得被 Claude Code 改写、润色、裁剪、摘要或删除。若玩家新输入与旧 AI 叙事、角色卡、记忆、变量或世界设定冲突，以玩家新输入为准；Claude Code 可以修正甚至重写 AI 生成的对话、摘要、角色资料、memory、变量和其他派生设定，使其符合玩家新设定。

玩家输入可能是三类或混合：第一人称下一步行动、第一人称接下来剧情梗概、第三人称上帝视角设定。若是行动，下文先简短复述玩家行动再推进剧情；若是剧情梗概，下文先结合已有数据扩写再视情况推进；若是上帝视角设定，视为权威数据并修正所有相关派生文件。历史输入编辑事件见 `PLAYER_INPUT_EDITS_PENDING`，需要评估并修正已生成数据，但仍不得改动玩家编辑后的原文。
