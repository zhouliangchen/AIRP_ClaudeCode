...
- Process a response directly: `python skills/frontend/handler.py "<card_folder>"`; add `--opening` for the initial opening turn.
...
Use `python -m unittest discover -s tests -v` for the repository test suite. `skills/package.json` still defines `npm test` as a placeholder, so do not use it as the primary verification command. For narrow Python edits, add targeted checks such as `python -m py_compile skills/<file>.py`.
...
- `skills/server.py` serves the frontend, accepts browser input, records authoritative player input in `.player_inputs.jsonl`, writes a pending user turn for immediate UI display, writes `skills/styles/input.txt`, and uses `.pending` as the signal Claude Code waits on. It binds to `0.0.0.0:8765` by default for LAN access; set `AIRP_HOST=127.0.0.1` to restrict it to local-only mode. It also exposes reroll, delete-turn, settings, opening, session-init, progress, and player-input edit APIs.
...
- `skills/import_prepare.py` is the startup pipeline. It cleans stale runtime state, delegates card/world import to `skills/importing/card.py`, initializes `.card_path`, `state.js`, `content.js`, `chat_log.json`, and writes `skills/styles/import_context.txt` as the single startup context file for Claude.
...
- Python orchestrates runtime agents that create and review narrative output through the `.agent_runs/<round>/` message runtime. `skills/llm/settings.py`, `skills/llm/provider.py`, and `skills/llm/runner.py` resolve cc_switch Claude Code local reverse proxy first, then OpenAI-compatible fallback for text agents. `skills/runtime/agent_messages.py` owns the append-only message bus, ACL/projection, and per-agent inbox indexes; `skills/runtime/agent_intents.py` owns executable control-plane requests; `skills/runtime/agent_snapshots.py` owns before-round snapshots; `skills/agents/shared/prompts.py` materializes prompts; `skills/agents/actor/runtime.py` owns shared actor collaboration helpers; `skills/runtime/agent_outputs.py` validates artifacts, assembles `story.input.json`, records critic revise/block repair history plus linked repair request messages/intents and related system improvement suggestions, and mirrors approved `story.output.json` content to `skills/styles/response.txt`. `skills/agents/postprocess/outputs.py` validates frontend-facing summary/options/goal data and records nonblocking UI-extension repairs. `round_deliver.py` performs word-count gating, appends token data, invokes `skills/frontend/handler.py`, updates memory, and reports whether story planning is due.
- `skills/runtime/round_runtime.py` and `skills/runtime/agent_runtime_pump.py` consume pending intents, explicitly execute projection, actor, subGM thread, story, critic, postprocess, repair, and delivery work, write authoritative artifacts under `artifacts/`, and block unsafe or stalled runs; `skills/control_plane_smoke.py` drives a temporary no-live-model control-plane smoke that covers artifacts, interaction trace, memory delta/summary, message runtime evidence, snapshot, and delivery.
...
- `skills/frontend/handler.py` parses tagged responses, executes MVU updates through `skills/domain/mvu_engine.py`, appends `chat_log.json`, rebuilds `content.js`, mirrors `state.js` into the card folder, and notifies the browser via `/api/done`.
- `skills/domain/mvu_engine.py`, `skills/importing/mvu_check.py`, `skills/mvu_shared.js`, and `skills/mvu_server.js` implement the variable/JSONPatch system and schema validation.
- `skills/importing/card.py`, `skills/importing/worldbook.py`, `skills/runtime/write_memory.py`, and `skills/agents/actor/memory.py` manage card parsing, worldbook lookup, persistent narrative memory, validated subagent memory deltas, and scheduled actor self-summaries under each card folder's `memory/` directory.
...
Important data flow: browser submit → `server.py` records `.player_inputs.jsonl`, writes `.pending_user_turn.json` with `role_text` / `user_instruction_text`, rebuilds `content.js`, writes `input.txt` + `.pending` + progress → Claude Code entry loop detects pending input and invokes the Python runtime → `round_prepare.py` writes `round_context.txt` and `.agent_runs/<round>/` packets/prompts/manifest, normalizing those fields into `role_channel` / `user_instruction_channel` in `input.json` and creating a before-round snapshot → `rp_generate_cli.py` calls runtime agents through `llm.runner.run_llm_agent()` instead of one-shot Claude CLI subprocesses → GM, subGM, story, critic, postprocess, player, and character agents collaborate through `messages.jsonl` and `intents/`; actor-facing delivery must pass through projection before reaching `inboxes/player.jsonl` or `inboxes/character_<id>.jsonl` → `round_runtime.py` and `agent_runtime_pump.py` consume pending intents as executable control-plane work; GM turn only declares actor calls, side-thread work, or stop reasons, then the runtime explicitly runs projection, actor, subGM thread, and GM continuation intents before story composition → runtime records `interaction.trace.json`, writes authoritative artifacts under `artifacts/`, and blocks unsafe or stalled runs → `story.input.json` is assembled with sanitized interaction trace; root same-name story/critic/postprocess files are delivery-boundary exports rather than control-plane authority → critic pass creates `run_postprocess`, and valid postprocess core creates the later `deliver_round` intent while UI-extension repairs remain nonblocking → `round_deliver.py` validates story/critic delivery artifacts, records critic revise/block `repair_history.jsonl`, repair request message/intent, and optional `.agent_runs/improvement_queue.jsonl`, mirrors approved story content to `response.txt`, and calls `frontend/handler.py` → `handler.py` clears the pending turn, updates chat/memory/state/progress, and rebuilds frontend content. Historical player edits go through `/api/player_inputs/edit`, append `.player_input_edits.jsonl`, and either update display in place or truncate the old branch and resubmit the revised input.
...
- 阶段 skill 名称：`rp`, `rp-orchestrator`, `rp-input-router`, `rp-context-projector`, `rp-delivery`, `rp-assets-ui`。
...
- 每轮使用 `.agent_runs/<round>/` 作为 agent 消息运行时：`messages.jsonl` 是 append-only 通信日志，`inboxes/` 是投影后的 per-agent 投递索引，`intents/` 是可执行控制面请求，`artifacts/` 是物化产物。默认 live path 由 `runtime/round_runtime.py` 驱动，串联 input analysis、GM collaboration、projection、actor、subGM、story、critic、postprocess、delivery 与 post-round memory；`runtime/agent_runtime_pump.py` 负责受控 intent 执行和能力请求消费。actor-facing 交付必须先经过 projection，才能进入 `inboxes/player.jsonl` 或 `inboxes/character_<id>.jsonl`；critic 通过后，默认 thin runtime 先执行 postprocess，只有 `postprocess.output.json.core` 通过校验后才进入 delivery；`round_deliver.py` 当前仍负责 story/critic 产物的机械交付、镜像 `story.output.json` 正文并调用前端处理。
...
- `{ROOT}/skills/frontend/handler.py`, `{ROOT}/skills/server.py`, `{ROOT}/skills/domain/mvu_engine.py`, `{ROOT}/skills/importing/mvu_check.py`, `{ROOT}/skills/importing/worldbook.py`, `{ROOT}/skills/runtime/write_memory.py`, `{ROOT}/skills/runtime/round_runtime.py`, `{ROOT}/skills/runtime/agent_runtime_pump.py`, `{ROOT}/skills/runtime/agent_messages.py`, `{ROOT}/skills/runtime/agent_intents.py`, `{ROOT}/skills/runtime/agent_snapshots.py`, `{ROOT}/skills/control_plane_smoke.py`, `{ROOT}/skills/agents/shared/prompts.py`, `{ROOT}/skills/runtime/agent_outputs.py`, `{ROOT}/skills/agents/actor/memory.py`, `{ROOT}/skills/agents/shared/schemas.py`, `{ROOT}/skills/round_prepare.py`, `{ROOT}/skills/round_deliver.py`, `{ROOT}/skills/import_prepare.py`, `{ROOT}/skills/start_server.py`
...
- 卡片文件夹（如 `{ROOT}/我的角色/`）下的 `chat_log.json` — handler.py 自动管理；可修正 AI 派生内容，但不得改写玩家原始输入
- 当前卡片文件夹下的 `.player_inputs.jsonl` — 玩家历次输入的权威日志；Claude Code 不得修改、裁剪或摘要，除非用户通过明确的玩家编辑功能触发
- 当前卡片文件夹下的 `.player_input_edits.jsonl`、`.player_input_branches.jsonl` — 玩家编辑审计和旧分支归档；只能由玩家编辑 API 写入，Claude Code 只能读取并据此修正派生数据
...
- `python "{ROOT}/skills/frontend/handler.py" "<卡片文件夹>" [--opening|--injections]` — 处理回合 / 开局 / 注入规则查询
- `python "{ROOT}/skills/importing/card.py" "<卡片文件夹>" "{ROOT}"` — 单独导入角色卡（兜底）
- `python "{ROOT}/skills/importing/worldbook.py" "<卡片文件夹>"` — 匹配变量变更与世界书索引
- `python "{ROOT}/skills/runtime/write_memory.py" "<卡片文件夹>"` — 追加本轮摘要到 project.md
- `python -c "import sys; sys.path.insert(0, r'{ROOT}/skills'); from runtime import agent_outputs; agent_outputs.build_story_input(r'<当前 .agent_runs 轮目录>')"` — 在 runtime 完成 GM/projection/actor/subGM 协作后防御性重建 `story.input.json`（通常由 `round_deliver.py` 调用）
...
- 单独导入（如需）：`python "{ROOT}/skills/importing/card.py" "<卡片文件夹>" "{ROOT}"`
...
1. 执行：`python "{ROOT}/skills/frontend/handler.py" "<卡片文件夹绝对路径>" --opening`
...
**玩家输入权威原则**。玩家历次输入是最高优先级事实源，`.player_inputs.jsonl` 中的 `raw_text` / `display_text` 不得被 Claude Code 改写、润色、裁剪、摘要或删除。系统每轮都要实时根据玩家最新输入评估过去的剧情和设定：若玩家输入与旧 AI 叙事、角色卡、记忆、变量或世界设定冲突，以玩家新输入为准；Claude Code 可以根据评估结论，对 AI 生成的对话、摘要、角色资料、memory、变量和其他派生设定进行小幅度修改，或在必要时完全重写，使其符合玩家新设定。

玩家输入可能是三类或混合：第一人称下一步行动、第一人称接下来剧情梗概、第三人称上帝视角设定。若是行动，下文先简短复述玩家行动再推进剧情；若是剧情梗概，下文先结合已有数据扩写再视情况推进；若是上帝视角设定，视为权威数据并修正所有相关派生文件。若玩家明确要求修改“第一段/首段/上一轮/前文/既有回复”等 AI 派生内容，必须把它当作派生内容编辑，定点修正既有 AI 回复或变量/记忆，而不是把指定内容演在最新回复里。若玩家手动指定“重要角色/核心角色”，必须沉淀为角色资料并加入 `character_orchestration.major`，本轮和后续相关场景优先使用该角色 subagent 获取独立反应。历史输入编辑事件见 `PLAYER_INPUT_EDITS_PENDING`，需要评估并修正已生成数据，但仍不得改动玩家编辑后的原文。
...
| `PLAYER_AUTHORITY_RULES` | 动态后缀 | 玩家输入权威规则和冲突处理边界 |
| `PLAYER_INPUT_INTERPRETATION` | 动态后缀 | 行动、剧情梗概、上帝视角设定和混合输入的处理规则 |
| `PLAYER_INPUT_HISTORY` | 动态后缀 | 最近玩家原始输入权威日志 |
| `PLAYER_INPUT_EDITS_PENDING` | 动态后缀 | 玩家历史输入编辑事件和待修正影响 |
...
**步骤 3** — 进入多 agent 创作链：输入路由 → 上下文投射 → GM/player/character subagents → story agent 整理 → critic agent 审核：
...
**普通回合（含用户输入）：**
```
<polished_input>可选：对玩家输入类型和处理方式的内部说明，不得替代、裁剪或润色玩家原文</polished_input>
<content>
<p>叙事正文段落...</p>
</content>
<character_dialogues>
...
每次 story agent 生成正文前，走完以下五步。内部思考，不写入 response.txt。
