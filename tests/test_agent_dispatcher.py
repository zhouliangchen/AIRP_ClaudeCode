...
class AgentDispatcherFoundationTest(unittest.TestCase):
    ...
    def test_request_projection_intent_projects_message_and_creates_run_actor(self):
        ...
        pending = self.intents.list_intents(self.run_dir, "pending")
        self.assertEqual([item["type"] for item in pending], ["run_actor"])
        projected = [
            message
            for message in self.dispatcher.agent_messages.read_messages(self.run_dir)
            if message.get("type") == "projected_message"
        ]
        self.assertEqual(len(projected), 1)
        self.assertEqual(projected[0]["to"], ["character:Ada"])

    @unittest.expectedFailure
    def test_run_actor_blocks_without_projected_message(self):
        created = self.intents.create_intent(
            self.run_dir,
            {
                ...
                "payload": {
                    "actor_id": "character:Ada",
                    "source_call_id": "call-character-Ada-1",
                    "projected_message_id": "msg_999999",
                },
            },
        )["intent"]
    ...
    @unittest.expectedFailure
    def test_run_subgm_thread_intent_writes_side_thread_artifact(self):
        created = self.intents.create_intent(
            self.run_dir,
            {
                "requested_by": "gm",
                ...
                "payload": {
                    "thread_id": "side-001",
                    "objective": "Resolve a hallway noise off-screen.",
                },
            },
        )["intent"]
        ...
        self.assertEqual(result["status"], "completed")
        self.assertEqual(result["intent_id"], created["id"])
        self.assertTrue((self.run_dir / "side_threads" / "side-001" / "state.json").exists())

    @unittest.expectedFailure
    def test_run_gm_turn_exposes_actor_dispatch_as_follow_up_intents(self):
        ...
        def fake_loop(run_dir, _manifest, _root, _run_claude, repair_context=None):
            ...
            _write_json(run_dir / "actor.outputs.json", {})
            return {"gm_steps": 1, "called_actors": ["character:Ada"]}
        ...
        self.assertEqual(result["intent_id"], created["id"])
        pending = self.intents.list_intents(self.run_dir, "pending")
        self.assertIn("request_projection", [item["type"] for item in pending])
        self.assertNotEqual([item["type"] for item in pending], ["compose_story"])
