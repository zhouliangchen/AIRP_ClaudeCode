...
class AgentPacketTest(unittest.TestCase):
    ...
    def test_prepare_agent_run_reads_root_actor_memory_files(self):
        ...
        self.assertIn('可进一步回忆"封存索引"：我知道它和 Ada 的灯有关', player_packet["memory"]["key_memories"])
        ...
        self.assertIn('可进一步回忆"第三层书架"：我看见玩家在那里停下', character_packet["memory"]["key_memories"])
    ...
    def test_prepare_agent_run_uses_safe_character_actor_id_in_context_and_prompt(self):
        ...
        self.assertIn(f"# 我的行动提示：{safe_name}", char_prompt)
        self.assertIn(f"我是 {safe_name}", char_prompt)
    ...
    def test_prepare_agent_run_writes_prompts_and_manifest(self):
        ...
        self.assertIn("现在：", char_prompt)
        self.assertNotIn("我能感知到的内容：", char_prompt)
        self.assertNotIn("刚刚对我说的话：", char_prompt)
        self.assertNotIn("我此刻延续的第一人称意图：", char_prompt)
