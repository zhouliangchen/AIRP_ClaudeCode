...
class AgentMemoryTest(unittest.TestCase):
    ...
    def test_post_round_memory_prompts_use_unified_file_backed_actor_context(self):
        ...
        self.assertIn('可进一步回忆"玩家重点"：我记得铜钥匙很重要。', player_prompt)
        self.assertNotIn("我想回忆：玩家重点", player_prompt)
        self.assertIn("玩家重点", player_prompt)
        self.assertIn("我记得铜钥匙很重要。", player_prompt)
        self.assertIn('可进一步回忆"Ada重点"：我记得旧档案室的封条。', ada_prompt)
        self.assertNotIn("我想回忆：Ada重点", ada_prompt)
